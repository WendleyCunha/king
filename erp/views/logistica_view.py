...
import pandas as pd
import datetime
from erp.services import logistica_service as svc
from erp.services import motorista_service, fiscal_service
...
def render(pode_editar: bool):
    ...
    aba1, aba2, aba3 = st.tabs(["Montar carga", "Cargas em andamento / faturamento", "Motoristas"])
    ...
    with aba2:
        ...
        if pode_editar and carga["status"] == "MONTAGEM":
            ...
            if motoristas:
                ...
                st.info("Cadastre motoristas em Configurações → Usuários (role motorista) antes de finalizar a carga.")
    ...
    with aba3:
        st.caption(
            "Motoristas são os mesmos usuários cadastrados no Painel (Configurações → "
            "Usuários, role motorista + placa) — não existe cadastro duplicado aqui."
        )
        dados = motorista_service.entregas_por_motorista()
        if dados:
            df = pd.DataFrame([{
                "Motorista": d["motorista"]["nome"], "Placa": d["motorista"].get("placa", "-"),
                "Login": d["motorista"]["usuario"], "Entregas (cargas finalizadas)": d["total_entregas"],
            } for d in dados])
            st.dataframe(df, use_container_width=True, hide_index=True)
        else:
            st.info("Nenhum motorista cadastrado no Painel ainda.")
