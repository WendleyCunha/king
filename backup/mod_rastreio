...
def renderizar_rastreio(df: pd.DataFrame, data_consulta: str, papel: str,
                        user: dict = None, datas_db: list = None, pode_exp: bool = False):
    ...
    with c1:
        termo = st.text_input("", placeholder="🔍  Buscar: placa, motorista, cliente, endereço, telefone, tracking...",
                              label_visibility="collapsed", key="busca")
    with c2:
        f_st = st.selectbox("", ["Todos os status","✅ Sucesso","❌ Falhou","📱 Notificado","⏳ Pendente","🚚 Em rota"],
                            label_visibility="collapsed", key="f_status")
    with c3:
        f_nt = st.selectbox("", ["Notificacao: Todas","Sim","Nao"],
                            label_visibility="collapsed", key="f_notif")

    df_f = aplicar_busca(df, termo)
    if f_st != "Todos os status": df_f = df_f[df_f["_status_visual"] == f_st]
    if f_nt == "Sim":  df_f = df_f[df_f["_notificado"] == True]
    elif f_nt == "Nao": df_f = df_f[df_f["_notificado"] == False]
    if termo and df_f.empty:
        st.warning(f"Nenhum resultado para **{termo}**."); return

    # ── Abas ──────────────────────────────────────────────────────
    abas_nomes = ["🏠 Dashboard", "🧑 Visao por Motorista"]
    ...
    with abas[0]:
        ...
        k6.markdown(f'<div class="kpi-card gold"><div class="kpi-label">🧑 Motoristas</div><div class="kpi-value">{motores}</div><div class="kpi-sub">Em operacao</div></div>', unsafe_allow_html=True)
        ...
        st.dataframe(pd.DataFrame({
            ...
            "Endereco": get_series(df_f,"address"),
            "Status":   df_f["_status_visual"],
            "Notif.":   df_f["_notificado"].apply(lambda x:"Sim" if x else "Nao"),
    ...
    with abas[1]:
        ...
        with col_d:
            ...
            <div style="background:#fff;border-left:6px solid #C9A84C;border-radius:10px;
                ...
                <span style="font-size:0.73rem;color:#64778d;">{ok}/{tot} concluidas ({pct}%)</span>
            </div>""", unsafe_allow_html=True)

            tf, tfl = st.tabs(["📋 Fila de Clientes","⚠️ Ocorrencias"])
            with tf:
                st.dataframe(pd.DataFrame({
                    "Ordem":    get_series(dr,"order"),
                    "Cliente":  get_series(dr,"title"),
                    "Endereco": get_series(dr,"address"),
                    "Status":   dr["_status_visual"],
                    "Notif.":   dr["_notificado"].apply(lambda x:"Sim" if x else "Nao"),
            ...
            with tfl:
                ...
                if df_err.empty: st.success("Nenhuma ocorrencia.")
                else:
                    st.dataframe(pd.DataFrame({
                        ...
                        "Horario":get_series(df_err,"checkout_time").apply(formatar_data),
                    }), use_container_width=True, hide_index=True)

    # ══ EXPORTAR (aba interna do Rastreio) ════════════════════════
    if pode_exp:
        with abas[2]:
            ...
            def montar(df_src):
                return pd.DataFrame({
                    ...
                    "Endereco":   get_series(df_src,"address"),
                    "Status":     get_series(df_src,"_status_visual","⏳ Pendente"),
                    "Notificado": get_series(df_src,"_notificado").apply(lambda x:"Sim" if x else "Nao"),
